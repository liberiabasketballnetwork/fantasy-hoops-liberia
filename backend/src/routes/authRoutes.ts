...
router.post("/register", async (req, res) => {
  try {
    ...
    const existing = await findRowByField("Users", "email", parsed.email);
    if (existing) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    // Admin login shortcut (single admin account via env vars)
    if (
    ...
    const user = await findRowByField("Users", "email", parsed.email);
